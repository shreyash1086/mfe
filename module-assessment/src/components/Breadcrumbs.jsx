...
const Breadcrumbs = ({ items, className = '', transparent = false }) => {
    return (
        <nav className={`flex ${className || 'mb-2'}`} aria-label="Breadcrumb">
            <ol className={`inline-flex items-center space-x-1 md:space-x-2 transition-colors duration-300 ${transparent ? 'bg-transparent px-0 py-0 border-none' : 'bg-white dark:bg-brand-card px-4 py-2 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700'}`}>
