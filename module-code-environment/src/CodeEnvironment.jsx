...
const tasks = [{ id: "task1", title: "Kode Environment" }];
      ...
      <Card hoverEffect={!loading}>
        <div className="flex flex-col items-center justify-center min-h-[170px]">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 text-sm text-center">
            Click to access your assessment environment
          </p>
        </div>
      </Card>
    </div>
...
const CodeEnvironment = () => {
  ...
  console.log(
    "[CodeEnvironment] Derived Cohort:",
    SOURCE_COHORT,
    "from user:",
    user?.username,
  );
  ...
  // ----------------------------- CONFIG -----------------------------
  const QUESCODE_API_BASE =
    process.env.QUESCODE_API_BASE ||
    (process.env.NODE_ENV === "development"
      ? "/quescode-fetch-api"
      : "https://64whx2c4ir65zu5l2ocpuobnxm0lpzph.lambda-url.ap-south-1.on.aws");
    ...
    const initializeCodes = async () => {
      try {
        ...
        if (listData.success && listData.files && listData.files.length > 0) {
          let targetFile = listData.files.includes("default")
            ? "default"
            : listData.files[0];
          ...
          if (!contentResp.ok) {
            ...
            throw new Error(
              `Content fetch failed ${contentResp.status}: ${text}`,
            );
          }
            ...
            tasks.forEach((task) => {
              if (availableCodes.length > 0) {
                const randomIndex = Math.floor(
                  Math.random() * availableCodes.length,
                );
  ...
  const handleGetQuestionCode = async () => {
    ...
    showToast(
      "No quescode assigned for this environment. Please contact admin.",
      "warning",
    );
  };
  ...
  const startProctoring = async () => {
    try {
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        showToast(
          "Camera/Screen access requires HTTPS. Please use a secure connection.",
          "error",
        );
        return false;
      }
  ...
  const captureAndUpload = async () => {
    try {
      ...
      if (
        ...
        screenVideoRef.current.videoHeight === 0
      ) {
      ...
      if (
        ...
        cameraVideoRef.current.videoHeight === 0
      ) {
  ...
  const handleStartEnv = async () => {
    ...
    try {
      ...
      if (!password) {
        showToast(
          "Session expired or password missing. Please relogin.",
          "error",
        );
        setKodeEnvLoading(false);
        return;
      ...
      if (!encodedId)
        throw new Error(
          "No connection ID returned. Backend response may be invalid.",
        );

      const guacToken =
        startData.token ||
        startData.authToken ||
        startData?.guacamole_setup?.response?.authToken;
                  ...
                  <div className="space-y-6 sm:space-y-8">
                    <div className="text-left">
                      <label
                        ...
                        className="block text-xs font-bold text-gray-800 dark:text-gray-200 mb-2 uppercase tracking-wide"
                      >
                        Candidate Name <span className="text-red-500">*</span>
                      </label>
                      <div className="relative group">
                        <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-brand-accent transition-colors">
                          person
                        </span>
                        <input
                          id="candidateName"
                    ...
                    <button
                      ...
                      className={`w-full py-4 text-white font-bold text-sm tracking-wide rounded-xl transition-all uppercase flex items-center justify-center gap-2 ${
                        !candidateName.trim() || kodeEnvLoading
                          ? "bg-brand-accent/40 cursor-not-allowed shadow-none"
                          : "bg-blue-600 hover:bg-blue-700 shadow-lg shadow-brand-accent/20 active:scale-[0.98]"
                      }`}
                    >
                      {showCode
                        ? "Regenerate Question Code"
                        : "Get Question Code"}
                    </button>
                      ...
                      {showCode && (
                        ...
                        >
                          <div className="text-left pt-6 pb-2 px-1">
                            <p className="text-xs font-bold text-gray-400 dark:text-slate-500 mb-2 uppercase tracking-wide">
            ...
            {toast.show && (
              ...
              ${
                toast.type === "error"
                  ...
                  : toast.type === "warning"
                    ? "bg-yellow-50/90 dark:bg-yellow-900/90 text-yellow-800 dark:text-white border-yellow-200 dark:border-yellow-800"
