...
function CohortContent() {
  ...
  useEffect(() => {
    const fetchCohorts = async () => {
      ...
      if (userRole === "trainer" && user?.username) {
        ...
        const filtered = cohortsData.filter(
          (c) =>
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${CONTENT_API_BASE_URL}${CONTENT_ENDPOINTS.LIST}?cohort=${encodeURIComponent(targetCohort)}&_t=${timestamp}`,
        {
          cache: "no-store",
        },
      );
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < totalFiles; i++) {
        ...
        for (const cohort of uploadConfig.cohorts) {
          for (const category of categoriesToUpload) {
            completed++;
            setUploadProgress(
              `Uploading ${completed}/${grandTotal}: ${file.name} to ${cohort} (${category})`,
            );
            ...
            const response = await fetch(
              `${CONTENT_API_BASE_URL}${CONTENT_ENDPOINTS.UPLOAD}?${queryParams.toString()}`,
              {
                method: "POST",
                body: formData,
              },
      ...
      alert(
        `Successfully uploaded ${totalFiles} file(s) across ${totalCohorts} cohort(s) and ${totalCategories} category(s)!`,
      );
      setUploadConfig({
        title: "",
        cohorts:
          effectiveCohortId && effectiveCohortId !== "unknown"
            ? [effectiveCohortId]
            : [],
        categories: [],
        description: "",
            ...
            {uploadConfig.files.length > 0 ? (
              <div className="space-y-3">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">
                    {uploadConfig.files.length} file(s) selected
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    {uploadConfig.files.map((f) => f.name).join(", ")}
                  </p>
                </div>
                <button
                  onClick={() => setUploadConfig((p) => ({ ...p, files: [] }))}
              ...
              <>
                <div className="p-3 bg-white dark:bg-white/5 shadow-sm rounded-lg mb-4">
                  <span className="material-symbols-outlined text-brand-accent text-2xl">
                    upload_file
                  </span>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setUploadConfig({ ...uploadConfig, title: e.target.value })
                }
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2 flex justify-between">
                ...
                {uploadConfig.cohorts.length > 0 && (
                  <span className="text-brand-accent normal-case font-bold">
                    {uploadConfig.cohorts.length} Selected
                  </span>
                )}
              </label>
              <div className="relative" ref={cohortRef}>
                ...
                >
                  ...
                  <span
                    className={`material-symbols-outlined transition-transform duration-200 ${isCohortDropdownOpen ? "rotate-180" : ""}`}
                  >
                    expand_more
                  </span>
                </div>
                  ...
                  <div className="absolute z-50 mt-2 w-full md:w-[150%] left-0 md:-left-[25%] max-h-80 overflow-y-auto bg-white dark:bg-brand-card border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl animate-in fade-in slide-in-from-top-2 duration-200 p-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      {cohorts.filter((c) =>
                        ...
                        cohorts
                          ...
                          .map((c) => (
                            ...
                            >
                              ...
                              <span
                                className={`text-sm ${uploadConfig.cohorts.includes(c.id) ? "text-brand-accent font-semibold" : "text-gray-700 dark:text-gray-300"}`}
                              >
                                {c.name}
                              </span>
                            </div>
                        ...
                        <div className="col-span-2 px-3 py-4 text-sm text-gray-500 text-center italic">
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {uploadConfig.cohorts.map((c) => (
                  <span
                    key={c}
                    className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-brand-accent/10 text-brand-accent border border-brand-accent/20 animate-in fade-in zoom-in duration-200"
                  >
                    <span className="material-symbols-outlined text-[14px] mr-1.5 text-brand-accent">
                      check_circle
                    </span>
                    {c}
                    <button
                      onClick={(e) => {
                        ...
                        setUploadConfig((prev) => ({
                          ...prev,
                          cohorts: prev.cohorts.filter((item) => item !== c),
                        }));
                      }}
          ...
          <div className="grid grid-cols-1 gap-6 mt-6">
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2 flex justify-between">
                ...
                {uploadConfig.categories.length > 0 && (
                  <span className="text-brand-accent normal-case font-bold">
                    {uploadConfig.categories.length} Selected
                  </span>
                )}
              </label>
              <div className="relative" ref={categoryRef}>
                ...
                >
                  ...
                  <span
                    className={`material-symbols-outlined transition-transform duration-200 ${isCategoryDropdownOpen ? "rotate-180" : ""}`}
                  >
                    expand_more
                  </span>
                </div>
                      ...
                      {categories.length > 0 ? (
                        categories.map((cat) => (
                          <div
                            ...
                            className={`flex items-center px-3 py-2.5 rounded-lg border transition-all cursor-pointer group
                                                         ${
                                                           uploadConfig.categories.includes(
                                                             cat,
                                                           )
                                                             ? "border-brand-accent bg-brand-accent/10"
                                                             : "border-transparent hover:bg-gray-50 dark:hover:bg-white/5"
                          ...
                          >
                            ...
                            <span
                              className={`text-sm ${uploadConfig.categories.includes(cat) ? "text-brand-accent font-semibold" : "text-gray-700 dark:text-gray-300"}`}
                            >
                              {cat}
                            </span>
                          </div>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {uploadConfig.categories.map((cat) => (
                  <span
                    key={cat}
                    className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 dark:bg-white/10 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-800 animate-in fade-in zoom-in duration-200"
                  >
                    <span className="material-symbols-outlined text-[14px] mr-1.5 text-indigo-500">
                      label
                    </span>
                    {cat}
                    <button
          ...
          <div className="flex flex-col md:flex-row items-center justify-between mt-8 pt-6 border-t border-gray-100 dark:border-gray-800 gap-4">
            ...
            <div className="flex gap-3 w-full md:w-auto justify-end">
              ...
              <button
                ...
                disabled={uploading}
                className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm rounded-xl shadow-sm shadow-brand-accent/20 transition-all flex items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {uploading ? (
                  <>
        ...
        {history.length > 0 && (
          <Card hoverEffect={false} gridBg={false} className="p-6 md:p-8 mt-6">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-6">
              Recent Uploads for {uploadConfig.cohorts[0]}
            </h3>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="text-sm">
                  {history.map((item, idx) => (
                    <tr
                      key={idx}
                      className="border-b border-gray-50 dark:border-white/5 last:border-0 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
                    >
                      ...
                      <td className="py-3 text-right pr-2">
                        <div className="flex justify-end gap-2">
                          <button
                            onClick={() =>
                              window.open(
                                `${CONTENT_API_BASE_URL}${CONTENT_ENDPOINTS.VIEW}?key=${encodeURIComponent(item.key)}`,
                                "_blank",
                              )
                            }
