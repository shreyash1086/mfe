...
function Content() {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (canSwitchCohorts) {
          ...
          if (inferred) {
            const matched = cohortsData.find((c) =>
              c.name
                .toLowerCase()
                .includes(inferred.replace(/-/g, " ").toLowerCase()),
            );
          ...
          if (inferred) {
            ...
            const prefixMatch = cohortsData.find((c) =>
              user?.username?.toLowerCase().startsWith(c.name.toLowerCase()),
            );
            if (prefixMatch) {
  ...
  useEffect(() => {
    ...
    const fetchContent = async () => {
      ...
      try {
        ...
        if (selectedCohort) {
          const response = await fetch(
            `${CONTENT_API_BASE_URL}${CONTENT_ENDPOINTS.LIST}?cohort=${encodeURIComponent(selectedCohort)}&_t=${timestamp}`,
            { cache: "no-store" },
          );
          if (response.ok) {
            const data = await response.json();
            fetchedFiles = (data.all_files || []).map((f) => ({
              ...f,
              _cohort: selectedCohort,
            }));
          }
        } else if (canSwitchCohorts) {
          const promises = cohorts.map((c) =>
            fetch(
              `${CONTENT_API_BASE_URL}${CONTENT_ENDPOINTS.LIST}?cohort=${encodeURIComponent(c.id)}&_t=${timestamp}`,
              { cache: "no-store" },
            )
              .then((res) => (res.ok ? res.json() : { all_files: [] }))
              .then((data) =>
                (data.all_files || []).map((f) => ({ ...f, _cohort: c.id })),
              )
              .catch(() => []),
          );
        ...
        processed.forEach((f) => {
          ...
          if (groupedMap.has(groupKey)) {
            ...
            if (f.cohortId && !existing.cohorts.includes(f.cohortId))
              existing.cohorts.push(f.cohortId);
  ...
  const handleView = async (key) => {
    try {
      const response = await fetch(
        `${CONTENT_API_BASE_URL}${CONTENT_ENDPOINTS.VIEW}?key=${encodeURIComponent(key)}`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const handleDeleteContent = (key, fileCohorts) => {
    if (!key) return;
    const confirmMsg =
      "Are you sure you want to PERMANENTLY delete this file and remove all its assignments? This cannot be undone.";
    showConfirm("Delete File", confirmMsg, () =>
      performDeleteContent(key, fileCohorts),
    );
  };
        ...
        await Promise.all(
          activeCohorts.map((cohortId) =>
            fetch(`${CONTENT_API_BASE_URL}${CONTENT_ENDPOINTS.DELETE}`, {
              ...
              console.error(`Wipe failed for cohort ${cohortId}`, err),
            ),
          ),
        );
      }
      const response = await fetch(
        `${CONTENT_API_BASE_URL}${CONTENT_ENDPOINTS.DELETE}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleAssignSubmit = async () => {
    ...
    try {
      ...
      for (const cohortId of cohortsToWipe) {
        ...
        if (canManageCohort) {
          await fetch(`${CONTENT_API_BASE_URL}${CONTENT_ENDPOINTS.DELETE}`, {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              s3_key: String(assignFileKey),
              cohort: String(cohortId),
            }),
          }).catch((err) =>
            console.error(`Sync cleanup failed for ${cohortId}`, err),
          );
        }
      }
      for (const category of targetCategories) {
        const response = await fetch(
          `${CONTENT_API_BASE_URL}${CONTENT_ENDPOINTS.ASSIGN}`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
      ...
      showAlert(
        "Sync Failed",
        "Failed to synchronize assignments: " + e.message,
      );
    } finally {
      setAssigning(false);
    }
  ...
  const toggleTargetCohort = (cohortId) =>
    setTargetCohorts((prev) =>
      prev.includes(cohortId)
        ? prev.filter((c) => c !== cohortId)
        : [...prev, cohortId],
    );
  const toggleTargetCategory = (categoryName) =>
  ...
  const getIcon = (type) => {
    ...
    if (["ipynb", "js", "html", "css", "py"].includes(type))
      return {
        icon: "code",
        ...
        actions={
          <div className="flex items-center gap-4">
            <div className="flex items-center px-4 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/10 py-1.5 max-w-xs group">
              <span className="material-symbols-outlined text-gray-400 group-focus-within:text-brand-accent transition-colors mr-2 text-xl">
                search
              </span>
              <input
                type="text"
              ...
              {canSwitchCohorts && (
                <div className="relative group w-full lg:w-auto">
                  ...
                  >
                    ...
                    <span
                      className={`material-symbols-outlined text-[20px] text-gray-400 transition-transform duration-200 ${isCohortDropdownOpen ? "rotate-180" : ""}`}
                    >
                      expand_more
                    </span>
                  </div>
              ...
              {isAuthorized && (
                <>
                  ...
                  <Link
                    to={
                      selectedCohort
                        ? `/content-uploading/cohort-content/${encodeURIComponent(selectedCohort)}`
                        : "/content-uploading/cohort-content"
                    }
                    className="flex-1 lg:flex-none px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-brand-accent/20 transition-all flex items-center justify-center gap-2"
                  >
                    <span className="material-symbols-outlined text-[20px]">
                      cloud_upload
                    </span>
              ...
              return (
                ...
                >
                  ...
                  <div className="col-span-1 flex justify-center md:justify-start">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center ${iconData.bg}`}
                    >
                  ...
                  <div className="col-span-2 flex justify-center md:justify-start gap-2">
                    {file.categories && file.categories.length > 0 ? (
                      ...
                      <span className="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-500 rounded-lg text-[10px] font-bold uppercase">
                        General
                      </span>
                    )}
                    <span
                      className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-brand-accent/10 text-brand-accent`}
                    >
                      {iconData.label}
                    </span>
                  </div>
                  ...
                  <div className="col-span-2 flex flex-wrap gap-1 justify-center md:justify-start">
                    {file.cohorts && file.cohorts.length > 0 ? (
                      file.cohorts.map((cid) => {
                        const cName =
                          cohorts.find((c) => c.id === cid)?.name || cid;
                        return (
                          <span
                            key={cid}
                            className="px-2 py-0.5 bg-indigo-50 dark:bg-white/5 text-indigo-600 dark:text-indigo-400 rounded text-[9px] font-bold"
                          >
                            {cName}
                          </span>
                        );
                  ...
                  <div className="col-span-2 w-full flex justify-center md:justify-end gap-2 opacity-100 transition-opacity">
                    ...
                    {isAuthorized && (
                      <>
                        <button
                          onClick={() => openAssignModal(file.key)}
                          className="p-2 rounded-lg text-gray-400 hover:text-brand-accent hover:bg-brand-accent/10 transition-colors"
                          title="Edit/Assign"
                        >
                          <span className="material-symbols-outlined text-[20px]">
                            add
                        ...
                        <button
                          onClick={() =>
                            handleDeleteContent(file.key, file.cohorts)
                          }
            ...
            {categoryLoading ? (
              ...
              <>
                  ...
                  <div className="mb-8">
                    ...
                    <div className="flex gap-4">
                      ...
                      <button
                        ...
                        className="px-6 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-xl transition-colors disabled:opacity-50"
                      >
                        Add
                      </button>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between items-end mb-4">
                      ...
                      <span className="text-xs font-bold text-gray-400">
                        Total:{" "}
                        {categories.filter((c) => c !== "All Resources").length}
                      </span>
                    </div>
                      ...
                      {categories
                        ...
                        .map((category) => {
                          return (
                            <div
                              key={category}
                              className="group flex items-center justify-between p-4 rounded-xl bg-gray-100 dark:bg-white/5 border border-transparent dark:border-brand-accent/10 hover:border-brand-accent/30 dark:hover:border-brand-accent/30 transition-all"
                            >
                ...
                <div className="px-8 py-6 bg-gray-50/50 dark:bg-black/20 flex justify-end gap-4 border-t border-gray-100 dark:border-white/5">
                  ...
                  <button
                    onClick={() => setCategoryModalOpen(false)}
                    className="px-8 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-brand-accent/20 transition-all"
                  >
                    Save Changes
                  </button>
                </div>
          ...
          <div className="bg-white dark:bg-brand-card rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden animate-in zoom-in-95 duration-300 border border-gray-100 dark:border-blue-500/20">
            <div className="p-6 border-b border-gray-100 dark:border-white/5 flex justify-between items-center bg-gray-50/50 dark:bg-black/20">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Manage which cohorts can access this resource
                </p>
              </div>
              <button
                onClick={() => setAssignModalOpen(false)}
                className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-200 dark:hover:bg-white/10 text-gray-400 hover:text-gray-600 dark:hover:text-white transition-all"
              >
                <span className="material-symbols-outlined text-[20px]">
                  close
                </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar">
                    {cohorts.map((cohort) => {
                      ...
                      const isInitiallyAssigned =
                        file &&
                        file.cohorts &&
                        file.cohorts.includes(cohort.name);
                      ...
                      return (
                        ...
                        >
                          <div className="relative flex items-center justify-center">
                            ...
                            />
                            <span className="material-symbols-outlined text-white text-[16px] absolute opacity-0 peer-checked:opacity-100 transition-all scale-75 peer-checked:scale-100 duration-200 pointer-events-none">
                              check
                            </span>
                          </div>
                          <span
                            className={`text-sm ${targetCohorts.includes(cohort.id) ? "text-brand-accent font-semibold" : "text-gray-700 dark:text-gray-300"}`}
                          >
                            {cohort.name}
                          </span>
                        </label>
                ...
                {categories
                  ...
                  .map((category) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-sm ${targetCategories.includes(category) ? "text-indigo-600 dark:text-indigo-400 font-semibold" : "text-gray-700 dark:text-gray-300"}`}
                      >
                        {category}
                      </span>
                    </label>
            ...
            <div className="p-6 bg-gray-50 dark:bg-black/20 flex justify-end gap-3">
              ...
              <button
                ...
                className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-brand-accent/20 transition-all flex items-center gap-2"
              >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-white dark:bg-brand-card rounded-2xl shadow-2xl w-full max-w-md overflow-hidden scale-100 animate-in zoom-in-95 duration-200 border border-gray-100 dark:border-white/10">
            <div className="p-6 text-center">
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${popup.type === "confirm" ? "bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400" : "bg-brand-accent/15 text-brand-accent"}`}
              >
              ...
              <div className="flex gap-3 justify-center">
                ...
                <button
                  ...
                  className={`px-6 py-2.5 rounded-xl text-white font-medium shadow-lg transition-transform active:scale-95 ${popup.type === "confirm" ? "bg-red-600 hover:bg-red-700 shadow-red-500/30" : "bg-blue-600 hover:bg-blue-700 text-white shadow-brand-accent/25"}`}
                >
