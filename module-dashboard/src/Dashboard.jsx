...
function Dashboard() {
  ...
  const candidateNotifications = [
    {
      id: 1,
      title: "New Assessment Assigned: SQL Basics",
      time: "10 mins ago",
      type: "info",
    },
    {
      id: 2,
      title: "Grades released for Python Module",
      time: "1 day ago",
      type: "success",
    },
    {
  ...
  const StatCard = ({ label, value, icon, bg, trend, trendDir }) => (
    <Card
      className="hover:border-brand-accent/20 dark:hover:border-brand-accent/30"
      accentColor="var(--brand-accent)"
    >
      <div className="flex justify-between items-start mb-4">
        <div
          className={`p-3 rounded-2xl ${bg} transition-transform group-hover:scale-110`}
        >
          {icon}
        </div>
        <div
  ...
  const ActivityItem = ({ title, time, type }) => {
    ...
    return (
      <div className="flex gap-4 relative pl-2 group">
        ...
        {/* Dot */}
        <div
          className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 z-10 ${colorMap[type] || "bg-gray-400"} ring-4 ring-white dark:ring-gray-900`}
        ></div>
        ...
        <div className="pb-6">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 leading-relaxed">
            Assessment submission recorded. Full report available in the
            analytics tab.
          </p>
  ...
  const AdminDashboard = () => (
    <div className="flex flex-col h-full gap-4">
        ...
        actions={
          userRole === "admin" && (
            <button
              ...
              className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-lg shadow-brand-accent/20 transition-all active:scale-95 whitespace-nowrap"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 shrink-0">
        {(userRole === "admin" ? adminStatsItems : candidateStatsItems).map(
          (stat, idx) => (
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="flex justify-between items-center mb-4 shrink-0">
            <div>
              ...
              <p className="text-gray-500 text-xs mt-1">
                Assessment engagement over the last 7 days
              </p>
            </div>
          </div>
        ...
        <Card
          ...
          accentColor="var(--brand-accent)"
        >
          <h3 className="font-bold text-lg text-gray-900 dark:text-white mb-4 shrink-0">
            Recent Activity
          </h3>
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide">
            ...
            {(notifications.length > 0
              ...
              <ActivityItem
                ...
                {...notif}
                time={
                  notif.time.includes("ago") ? notif.time : timeAgo(notif.time)
                }
              />
            ))}
          </div>
