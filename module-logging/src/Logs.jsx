...
import {
    Activity, Clock, AlertCircle, RefreshCw, Download, Filter,
    Search, Server, Users, ShieldAlert, CheckCircle2, TrendingUp, X, Cpu,
...
const COHORT_API_URL = 'https://x6uz5z6ju2.execute-api.us-west-2.amazonaws.com/SQLAdmin?type=cohorts';
...
function Logs() {
    ...
    const inferLevel = useCallback((log) => {
        try {
            const data = typeof log.req_res_data === 'string' ? JSON.parse(log.req_res_data) : (log.req_res_data || {});
            if (data?.status >= 500 || data?.error) return 'ERROR';
            if (data?.status >= 400) return 'WARN';
            const apiLower = (log.api_called || '').toLowerCase();
            if (apiLower.includes('admin') || apiLower.includes('login') || apiLower.includes('auth')) return 'SECURITY';
            return 'INFO';
        } catch {
            return 'INFO';
        }
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (!event.target.closest('[data-cohort-dropdown]')) setIsCohortDropdownOpen(false);
            if (!event.target.closest('[data-time-dropdown]')) {
                setIsTimeDropdownOpen(false);
            }
            if (!event.target.closest('[data-sort-dropdown]')) setIsSortDropdownOpen(false);
            if (!event.target.closest('[data-level-dropdown]')) setIsLevelDropdownOpen(false);
            if (!event.target.closest('[data-api-dropdown]')) setIsApiDropdownOpen(false);
        };
    ...
    const uniqueApis = useMemo(() => {
        ...
        const registryIds = new Set(API_REGISTRY.map(a => a.id).filter(id => id !== 'ALL' && id !== 'OTHER'));
        
        // Find existing logs that don't match registry
        const loggedApis = [...new Set(logs.map(log => log.api_called))].filter(Boolean);
    ...
    const filteredLogs = useMemo(() => {
        let result = logs.filter(log => {
            ...
            if (levelFilter !== 'ALL' && inferLevel(log) !== levelFilter) return false;
            if (searchTerm) {
                const term = searchTerm.toLowerCase();
                return (
                    (log.user_id || '').toLowerCase().includes(term) ||
                    (log.api_called || '').toLowerCase().includes(term) ||
                    JSON.stringify(log.req_res_data || '').toLowerCase().includes(term)
                );
            }
            return true;
        });
        ...
        return result;
    }, [logs, searchTerm, apiFilter, levelFilter, sortOrder, timeFilter, inferLevel]);
    ...
    const handleExport = () => {
        ...
        const headers = ['User ID', 'Timestamp', 'API Called', 'Request/Response Data'];
    ...
    const filteredCohorts = useMemo(() => {
        ...
        return cohorts.filter(c => c.name.toLowerCase().includes(cohortSearch.toLowerCase()));
    ...
    const usageStats = useMemo(() => {
        ...
        const loginCount = logs.filter(l => (l.api_called || '').includes('login')).length;
        return [
            {
        ...
        <Card className="p-4 rounded-2xl relative overflow-hidden group">
            <div className="relative z-10">
                <div className="flex justify-between items-start mb-2">
                    <div className={`p-2 rounded-xl ${bg} transition-transform group-hover:scale-110`}>
                        {icon}
                    </div>
            ...
            <div className="px-4 pt-2 pb-8 w-full h-full flex flex-col mx-auto overflow-hidden">
                ...
                <div className="flex-1 flex flex-col min-h-0 bg-transparent py-2 overflow-hidden">
                    ...
                    <div className="flex-1 flex flex-col bg-white dark:bg-brand-card rounded-[32px] shadow-sm border border-gray-100 dark:border-white/10 overflow-hidden relative min-h-0">
                        ...
                        <div className="absolute inset-0 opacity-[0.02] dark:opacity-[0.04] pointer-events-none"
                            style={{ backgroundImage: 'radial-gradient(circle, #06b6d4 1px, transparent 1px)', backgroundSize: '24px 24px' }}></div>
                        ...
                        <div className="p-6 border-b border-gray-100 dark:border-white/10 relative z-30 flex flex-col lg:flex-row gap-4 items-center bg-gray-50/50 dark:bg-black/20">
                            ...
                            <div className="flex-1 flex flex-col sm:flex-row gap-3 w-full lg:w-auto mt-1">
                                ...
                                <div className="relative w-full sm:w-56 flex-shrink-0" data-cohort-dropdown>
                                    <div
                                        ...
                                        <div className="flex items-center gap-2 truncate">
                                            ...
                                            <span className="truncate">
                                                {cohort
                                                    ...
                                                    : (cohortsLoading ? 'Loading...' : 'Select Cohort')}
                                            </span>
                                        </div>
                                        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform duration-200 flex-shrink-0 ${isCohortDropdownOpen ? 'rotate-180' : ''}`} />
                                    </div>
                                            ...
                                            <div className="max-h-[250px] overflow-y-auto scrollbar-hide p-1">
                                                {filteredCohorts.length > 0 ? (
                                                    filteredCohorts.map(c => (
                                                        <div
                                                            key={c.id}
                                                            onClick={() => { setCohort(c.id); setIsCohortDropdownOpen(false); setCohortSearch(''); }}
                                                            className={`px-3 py-2.5 rounded-lg text-sm cursor-pointer truncate transition-colors ${cohort === c.id
                            ...
                            <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full lg:w-auto">
                                ...
                                <div className="relative flex-1 sm:w-36" data-time-dropdown>
                                    ...
                                    >
                                        <div className="flex items-center gap-2 truncate">
                                            ...
                                            <span className="truncate">
                                                {timeFilter === 'ALL' ? 'Time: All' : `Last ${timeOptions.find(o => o.id === timeFilter)?.label || timeFilter}`}
                                            </span>
                                        </div>
                                        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform duration-200 flex-shrink-0 ${isTimeDropdownOpen ? 'rotate-180' : ''}`} />
                                    </div>
                                    ...
                                    {isTimeDropdownOpen && (
                                        <div className="absolute top-full left-0 mt-2 w-64 bg-white dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xl p-4 z-50 animate-in fade-in zoom-in-95 duration-200">
                                            <h4 className="text-sm font-bold text-gray-900 dark:text-white mb-4 px-1">Timespan</h4>
                                            <div className="space-y-1">
                                                {timeOptions.map(opt => (
                                                    <label
                                                        key={opt.id}
                                                        onClick={() => { setTimeFilter(opt.id); setIsTimeDropdownOpen(false); }}
                                                    ...
                                                    >
                                                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${timeFilter === opt.id ? 'border-brand-accent' : 'border-gray-300 dark:border-gray-600 group-hover:border-brand-accent/50'}`}>
                                                            {timeFilter === opt.id && <div className="w-2.5 h-2.5 rounded-full bg-brand-accent animate-in zoom-in-50 duration-200" />}
                                                        </div>
                                                        <span className={`text-sm font-semibold ${timeFilter === opt.id ? 'text-brand-accent' : 'text-gray-600 dark:text-gray-400'}`}>{opt.label}</span>
                                                    </label>
                                                ))}
                                            </div>
                                        </div>
                                ...
                                <div className="relative flex-1 sm:w-32" data-sort-dropdown>
                                    ...
                                    >
                                        ...
                                        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform duration-200 ${isSortDropdownOpen ? 'rotate-180' : ''}`} />
                                    </div>
                                    {isSortDropdownOpen && (
                                        <div className="absolute top-full left-0 mt-2 w-44 bg-white dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200">
                                            {[
                                                ...
                                                <div
                                                    key={opt.id}
                                                    onClick={() => { setSortOrder(opt.id); setIsSortDropdownOpen(false); }}
                                ...
                                <div className="relative flex-1 sm:w-28" data-level-dropdown>
                                    <div
                                        ...
                                        className="flex items-center justify-between gap-1.5 bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 text-xs sm:text-sm font-semibold rounded-xl px-3 py-2.5 cursor-pointer hover:border-brand-accent/30 dark:hover:border-brand-accent/50 transition-all shadow-sm"
                                    >
                                        <span className="truncate">{levelFilter === 'ALL' ? 'Flag: All' : levelFilter}</span>
                                        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform duration-200 ${isLevelDropdownOpen ? 'rotate-180' : ''}`} />
                                    </div>
                                    {isLevelDropdownOpen && (
                                        <div className="absolute top-full left-0 mt-2 w-40 bg-white dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200">
                                            {['ALL', 'INFO', 'WARN', 'ERROR', 'SECURITY', 'DEBUG'].map(level => (
                                                <div
                                                    key={level}
                                                    onClick={() => { setLevelFilter(level); setIsLevelDropdownOpen(false); }}
                                ...
                                <div className="relative flex-1 sm:w-48" data-api-dropdown>
                                    ...
                                    >
                                        <span className="truncate">
                                            {apiFilter === 'ALL' ? 'API: All Services' : (API_REGISTRY.find(a => a.id === apiFilter)?.label || apiFilter)}
                                        </span>
                                        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform duration-200 ${isApiDropdownOpen ? 'rotate-180' : ''}`} />
                                    </div>
                                    {isApiDropdownOpen && (
                                        <div className="absolute top-full left-0 mt-2 w-64 bg-white dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200 py-1 max-h-80 overflow-y-auto">
                                            ...
                                            {API_REGISTRY.map(opt => (
                                                <div
                                                    key={opt.id}
                                                    onClick={() => { setApiFilter(opt.id); setIsApiDropdownOpen(false); }}
                                            ...
                                            {uniqueApis.length > 0 && (
                                                <div className="border-t border-gray-100 dark:border-white/5 mt-1 pt-0.5">
                                                    {uniqueApis.map(api => (
                                                        <div
                                                            key={api}
                                                            onClick={() => { setApiFilter(api); setIsApiDropdownOpen(false); }}
                                ...
                                {(searchTerm || apiFilter !== 'ALL' || levelFilter !== 'ALL' || sortOrder !== 'desc' || timeFilter !== 'ALL') && (
                                    <button
                                        onClick={() => {
                                            setSearchTerm('');
                                            setApiFilter('ALL');
                            ...
                            {loading && logs.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-64 space-y-4">
                                    ...
                                    <div className="space-y-2 text-center">
                                        <p className="text-sm font-bold text-gray-500 dark:text-gray-400">Synchronizing Telemetry...</p>
                                ...
                                <table className="w-full text-left border-collapse min-w-[800px]">
                                    <thead>
                                        <tr className="bg-gray-50 dark:bg-[#1a1a1a] text-gray-400 dark:text-gray-500 text-[10px] font-bold uppercase tracking-wider border-b border-gray-100 dark:border-white/5">
                                            ...
                                            <th className="px-6 py-4 font-extrabold">Target Resource</th>
                                            <th className="px-6 py-4 font-extrabold">Telemetry Payload</th>
                                        </tr>
                                    </thead>
                                        ...
                                        <AnimatePresence mode="popLayout">
                                            {filteredLogs.length > 0 ? (
                                                filteredLogs.map((log, index) => {
                                                    let parsedData = null;
                                                    try {
                                                        parsedData = typeof log.req_res_data === 'string' ? JSON.parse(log.req_res_data) : (log.req_res_data || {});
                                                    } catch {
                                                        // ignore
                                                    }

                                                    const method = parsedData?.method || log.method || 'GET';
                                                        ...
                                                        >
                                                            <td className="px-6 py-4 whitespace-nowrap align-middle">
                                                                <div className="flex items-center gap-3">
                                                                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold border transition-all ${getLevelColor(levelStr)}`}>
                                                                        {levelStr}
                                                                    </span>
                                                            ...
                                                            <td className="px-6 py-4 whitespace-nowrap align-middle">
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <span className="text-[13px] font-bold text-slate-800 dark:text-slate-200">{log.user_id || '-'}</span>
                                                                </div>
                                                            </td>
                                                            <td className="px-6 py-4 whitespace-nowrap align-middle">
                                                                <div className="flex items-center gap-2">
                                                                    <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold uppercase ${getMethodColor(method)}`}>
                                                                        {method}
                                                                    </span>
                                                            ...
                                                            <td className="px-6 py-4 align-middle max-w-lg">
                                                                <code className="text-xs text-slate-600 dark:text-slate-400 font-mono block truncate group-hover:whitespace-normal group-hover:break-all transition-all max-h-32 overflow-y-auto">
                                                                    {formatReqRes(log.req_res_data || log.req_res_body)}
                                                                </code>
                                                            </td>
                                                        </motion.tr>
                                                    );
                                                ...
                                                <tr>
                                                    <td colSpan="4" className="px-6 py-20 text-center">
                                                        <div className="flex flex-col items-center justify-center text-gray-400 dark:text-gray-500">
                                                            ...
                                                            <p className="text-lg font-bold text-gray-600 dark:text-gray-300">No logs match criteria</p>
                                                            <p className="text-sm mt-1">Adjust search terms or clear filters to see more results.</p>
                                                        </div>
                                                    </td>
                                                </tr>
                                            )}
                ...
                <div className="px-6 py-4 border-t border-gray-100 dark:border-white/10 bg-gray-50/80 dark:bg-white/[0.02] flex items-center justify-between z-20 shrink-0 mt-auto">
                    <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                        {cohort ? (
                            <>
                                Displaying <strong className="text-gray-900 dark:text-white">{filteredLogs.length}</strong> matching records out of <strong className="text-gray-900 dark:text-white">{logs.length}</strong> total
                                {' · '}
                                <span className="text-xs text-gray-400">Cohort: <strong className="text-brand-accent">{cohort}</strong></span>
                            </>
                        ) : (
